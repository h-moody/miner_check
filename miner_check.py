...
parser.add_argument('--e9_ips', type=str, required=True, nargs='+', help='List of E9 Pro miner IP addresses')
parser.add_argument('--l7_ips', type=str, required=True, nargs='+', help='List of L7 miner IP addresses')
parser.add_argument('--wallet_e9', type=str, required=True, help='Wallet address for E9 Pro miners')
...
miners = {
    "E9 Pro": {
        ...
        "max_temp": 84,
        "min_hashrate_5s": 100,
        "min_hashrate_30m": 3300
    },
    "L7": {
        "urls": [f"http://{ip}" for ip in args.l7_ips],
        "max_temp": 80,
        "min_hashrate_5s": 100,
        "min_hashrate_30m": 8000
    }
}
...
for miner_type, miner_data in miners.items():
    for url in miner_data["urls"]:
        if miner_type == "L7":
            if args.wallet_l7:
                expected_wallet_address = args.wallet_l7
            elif args.wallet_prefix_l7:
                ip = extract_ip(url)
                expected_wallet_address = f"{args.wallet_prefix_l7}.{ip.split('.')[-1]}"
            else:
                raise ValueError("Either --wallet_l7 or --wallet_prefix_l7 must be provided for L7 miners.")
        else:
            expected_wallet_address = expected_wallet_addresses[miner_type]
